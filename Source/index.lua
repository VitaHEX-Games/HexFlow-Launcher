...
function listDirectory(dir)
	...
	for i,file in pairs(dir) do
		...
		if pcall(loadImage, custom_path) then
			-- load custom cover from app name
			if System.doesFileExist(custom_path) then
			imgTmp = Graphics.loadImage(custom_path)
			file.icon=imgTmp
			table.insert(files_table, 4,file.icon)
			end
		elseif pcall(loadImage, custom_path_id) then
			-- load custom cover from app id
			if System.doesFileExist(custom_path_id) then
			imgTmp = Graphics.loadImage(custom_path_id)
			file.icon=imgTmp
			table.insert(files_table, 4,file.icon)
			end
		else
			if not pcall(loadImage, img_path) then 
				-- no icon use blank
				imgTmp = imgCoverTmp
				file.icon=imgTmp
				table.insert(files_table, 4,file.icon)
				...
				-- load app icon
				if System.doesFileExist(img_path) then
...
-- Graphics
	...
	Graphics.fillRect(0, 960, 496, 544, themeCol)-- blue footer bottom
...
-- MAIN VIEW
    ...
    -- Header
        ...
        Font.setPixelSizes(fnt, 22)
        Font.print(fnt, 726, 34, string.format("%02d:%02d",h,m), white)-- Draw time
        life = System.getBatteryPercentage()
        Font.print(fnt, 830, 34, life .. "%", white)-- Draw battery
	...
	-- Footer buttons and icons
		...
		Font.print(fnt, 878, 508, "Launch", white)
		Graphics.drawImage(750, 510, btnT)
		Font.print(fnt, 778, 508, "Details", white)
		Graphics.drawImage(620, 510, btnS)
		Font.print(fnt, 648, 508, "Category", white)
		Graphics.drawImage(530, 510, btnO)
		Font.print(fnt, 558, 508, "View", white)
		
		Font.setPixelSizes(fnt, 22)
		if showCat == 1 then
			Font.print(fnt, 32, 34, "GAMES", white)
		elseif showCat == 2 then
			Font.print(fnt, 32, 34, "HOMEBREWS", white)
		elseif showCat == 3 then
			Font.print(fnt, 32, 34, "PSP", white)
		else
			Font.print(fnt, 32, 34, "ALL", white)
		end
		...
		if showView ~= 2 then
			...
			PrintCentered(fnt, 480, 427, app_title, white,22)-- Draw title
		else
			Font.print(fnt, 24, 508, app_title, white)
		end
    ...
    -- Draw Covers
		...
		if showCat == 1 then
			for l, file in pairs(games_table) do
				...
				if base_x>base_x-8 and base_x<base_x+8 then
					DrawCover((targetX+l*space)-(#games_table*space+space),-0.6,file.name, file.icon, base_x)--draw visible covers only
				end
			end
			if showView ~= 2 then
				PrintCentered(fnt, 480, 460, p .. " of " .. #games_table, white, 20)-- Draw total items
			end
		elseif showCat == 2 then
		--HOMEBREWS
			...
			if showView ~= 2 then
				PrintCentered(fnt, 480, 460, p .. " of " .. #homebrews_table, white, 20)-- Draw total items
			end
		elseif showCat == 3 then
		--PSP
			...
			if showView ~= 2 then
				PrintCentered(fnt, 480, 460, p .. " of " .. #psp_table, white, 20)-- Draw total items
			end
		else
		--ALL
			...
			if showView ~= 2 then
				PrintCentered(fnt, 480, 460, p .. " of " .. #files_table, white, 20)-- Draw total items
			end
		end
...
-- PREVIEW
	...
	-- Footer buttons and icons
		...
		Font.print(fnt, 878, 508, "Close", white)
				
		Graphics.fillRect(24, 450, 24, 450, themeCol)
		...
		Font.setPixelSizes(fnt, 22)
        Font.print(fnt, 50, 190, txtname, white)-- app name
        Font.print(fnt, 50, 240, "App ID: " .. app_titleid .. "\nVersion: " .. app_version, white)-- Draw info
		...
		if string.match(app_titleid, "PCS") and not string.match(app_titleid, "PCSI") then
			Render.drawModel(modCoverNoref,  prevX, -1.0, -5+prevZ, 0, math.deg(prevRot), 0)
			Render.drawModel(modBoxNoref,  prevX, -1.0, -5+prevZ, 0, math.deg(prevRot), 0)
		elseif string.match(app_titleid, "PSPEMU") and not string.match(app_titleid, "PSPEMUCFW") then
			Render.drawModel(modCoverPSPNoref,  prevX, -1.0, -5+prevZ, 0, math.deg(prevRot), 0)
			Render.drawModel(modBoxPSPNoref,  prevX, -1.0, -5+prevZ, 0, math.deg(prevRot), 0)
		else
			Render.drawModel(modCoverHbrNoref,  prevX, -1.0, -5+prevZ, 0, math.deg(prevRot), 0)
		end
...
-- SETTINGS
	-- Footer buttons and icons
		...
		Font.print(fnt, 778, 508, "Select", white)
		Graphics.fillRect(60, 900, 24, 472, grey)-- grey bg
		Font.setPixelSizes(fnt, 25)
        Font.print(fnt, 84, 42, "SETTINGS", white)-- Draw info
		...
		Font.setPixelSizes(fnt, 22)
		Font.print(fnt, 84, 152, "Startup Category: ", white)
			if startCategory == 0 then
				Font.print(fnt, 84+260, 152, "ALL", white)
			elseif startCategory == 1 then
				Font.print(fnt, 84+260, 152, "GAMES", white)
			elseif startCategory == 2 then
				Font.print(fnt, 84+260, 152, "HOMEBREWS", white)
			elseif startCategory == 3 then
				Font.print(fnt, 84+260, 152, "PSP", white)
			end
		
		Font.print(fnt, 84, 152+40, "Reflection Effect: ", white)
			if setReflections == 1 then
				Font.print(fnt, 84+260, 152+40, "ON", white)
			else
				Font.print(fnt, 84+260, 152+40, "OFF", white)
			end

		Font.print(fnt, 84, 152+80, "Sounds: ", white)
			if setSounds == 1 then
				Font.print(fnt, 84+260, 152+80, "ON", white)
			else
				Font.print(fnt, 84+260, 152+80, "OFF", white)
			end

		Font.print(fnt, 84, 152+120, "Theme Color: ", white)
			if themeColor == 1 then
				Font.print(fnt, 84+260, 152+120, "Red", white)
			elseif themeColor == 2 then
				Font.print(fnt, 84+260, 152+120, "Yellow", white)
			elseif themeColor == 3 then
				Font.print(fnt, 84+260, 152+120, "Green", white)
			elseif themeColor == 4 then
				Font.print(fnt, 84+260, 152+120, "Grey", white)
			elseif themeColor == 5 then
				Font.print(fnt, 84+260, 152+120, "Black", white)
			else
				Font.print(fnt, 84+260, 152+120, "Blue", white)
			end
			
		Font.print(fnt, 84, 152+160, "Custom Background: ", white)
			if setBackground == 1 then
				Font.print(fnt, 84+260, 152+160, "ON", white)
			else
				Font.print(fnt, 84+260, 152+160, "OFF", white)
			end
			
		Font.print(fnt, 84, 152+200, "About", white)
...
-- ABOUT
	-- Footer buttons and icons
		...
		.. "\n\nCredits\nProgramming/UI\nSakis RG\n\nDeveloped with Lua Player Plus by\nRinnegatamante", white)-- Draw info
